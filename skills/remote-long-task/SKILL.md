...
### 远端 python 一律 `python -u`（2026-08-13 固化，已踩 2 次）

- 经 ssh/管道执行 python 脚本时 stdout 是**块缓冲**——print 不实时显示，用户看不到进度以为卡住而 abort（踩坑：01:54 task_id 被缓冲吞掉、22:26 出片进度被吞）
- **任何远端长任务 python 脚本执行命令必须带 `-u`**（无缓冲）：`python -u script.py`
- 脚本内轮询循环的进度打印配合 -u 实时可见

### 提交与等待分离（长生成任务）

- 长生成任务（出片等）：**先提交（快速返回 task_id/queue 确认）→ 返回汇报 → 等待用 wait_until/拆段**（带进度可见性）
- ❌ 禁止"提交+长轮询循环"合并在一个脚本里单次 ssh 阻塞（用户看不到中间状态）
- ✅ 正确：提交后立即返回"已提交 + 预计时长"，再用 `wait_until`（每 60s 存活信号）拆段等完成
...
- **超长/用户外出**（>30min）：使用 Codex 当前可用的后台执行与等待机制配合 exec_remote，完成后写本地日志
...
2. **禁止单次 ssh 阻塞 >30 分钟**——超过就拆段或转为当前 Codex 环境支持的后台任务
3. **用户中止等待 ≥2 次 = 信号**：等待方式不对（阻塞过长/体验差），立即切换到分阶段短等待或后台任务，不要第三次使用相同的长阻塞
4. **用户中止单次等待后：立即做一次性 DONE 检测并返回**（2026-08-13 补充）——中止等待不等于任务失败，远端任务可能已经或即将完成。正确动作序列：中止等待 → 一次性检查 DONE 标记/日志 → 已完成则返回结果，未完成则转后台任务或拆段。不要停住等待用户再次追问
4. 踩坑实例（2026-08-13）：预热+加载合并 55 分钟阻塞被用户 abort 3 次；正确做法 = 预热一段（10min）返回汇报 → 加载一段（15min）返回汇报
5. **可变时长任务（pip 装包/下载）必须分段反馈**：这类任务耗时不确定。等待期间使用当前 Codex 的等待机制，每次不超过 60 秒，并按实际进度向用户更新；长任务转为后台执行
...
- `$` 在 PowerShell 双引号里会被插值；不要依赖 `\$` 转义。改为使用 `apply_patch` 创建 `.sh`/`.ps1` 文件，再用 scp 上传
- 内联 `ssh "python -c '...'"` 多层引号极易崩 -> 一律写脚本文件
- 中文注释在 PowerShell 5.1 可能解析报错 -> 脚本用英文/ASCII

## ⚠️ 远程命令执行铁律（2026-08-10 固化，反复踩坑）

**复杂远程命令一律使用三段式：本地用 `apply_patch` 创建 `.sh` → 单独 scp 上传 → ssh 只执行极简启动命令。不要内联复杂脚本。**

```
1. 本地用 `apply_patch` 创建远端脚本（bash 语法，UTF-8）
